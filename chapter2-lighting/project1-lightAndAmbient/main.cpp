...
int main() {
	...
	glm::vec3 rice_position(0.0f, 1.0f, 1.0f); //米浴立方体的世界坐标系位置
	...
	Shader shader2("vertexShader.vs", "lightShader.fs");
	glBindVertexArray(0);
	...
	while (!glfwWindowShouldClose(mainWindow)) {
		...
		unsigned int objectLoc = glGetUniformLocation(shader1.ID, "objectColor");
		unsigned int lightLoc = glGetUniformLocation(shader1.ID, "lightColor");
		glUniform3f(objectLoc, 1.0f, 0.5f, 0.31f);
